...
export class UsersProfileComponent implements OnInit, OnDestroy {
  ...
  getPlotById() {
    const sus = this.plotsService.getPlotById(this.authService.getUser().id).subscribe({
  ...
  updateUser() {
    ...
    const sus = this.usersService.putUser(updatedUser, this.authService.getUser().id).subscribe({
      next: (response) => {
        Swal.fire({
